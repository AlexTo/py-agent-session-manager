# @my-py-agents/strands-s3-infra
This library was generated with [@aws/nx-plugin](https://github.com/awslabs/nx-plugin-for-aws/).

## Building

Run `pnpm nx build @my-py-agents/strands-s3-infra [--skip-nx-cache]` to build the application.

## Running unit tests

Run `pnpm nx test @my-py-agents/strands-s3-infra` to execute the unit tests via Vitest.

### Updating snapshots

To update snapshots, run the following command:

`pnpm nx test @my-py-agents/strands-s3-infra --configuration=update-snapshot`

## Run lint

Run `pnpm nx lint @my-py-agents/strands-s3-infra`

### Fixable issues

You can also automatically fix some lint errors by running the following command:

`pnpm nx lint @my-py-agents/strands-s3-infra --configuration=fix`

## Deploy to AWS

### Deploy your Sandbox Stage

Run `pnpm nx deploy-sandbox @my-py-agents/strands-s3-infra`

### Deploy all Stacks in a Stage

Run `pnpm nx deploy @my-py-agents/strands-s3-infra "[stageName]/*"`

### Deploy a single Stack

Run `pnpm nx deploy @my-py-agents/strands-s3-infra [stageName]/[stackName]`
...
Run `pnpm nx deploy-sandbox @my-py-agents/strands-s3-infra --hotswap`
