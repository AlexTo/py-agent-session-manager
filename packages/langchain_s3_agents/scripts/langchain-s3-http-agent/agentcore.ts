// Resolves the deployed LangchainS3HttpAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['LangchainS3HttpAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'LangchainS3HttpAgent' found in runtime configuration (application ${application}).`,
    );
  }
