// Resolves the deployed LangchainS3AguiAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['LangchainS3AguiAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'LangchainS3AguiAgent' found in runtime configuration (application ${application}).`,
    );
  }
