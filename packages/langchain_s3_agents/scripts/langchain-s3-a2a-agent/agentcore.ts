// Resolves the deployed LangchainS3A2aAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['LangchainS3A2aAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'LangchainS3A2aAgent' found in runtime configuration (application ${application}).`,
    );
  }
