// Resolves the deployed StrandsS3AguiAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['StrandsS3AguiAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'StrandsS3AguiAgent' found in runtime configuration (application ${application}).`,
    );
  }
