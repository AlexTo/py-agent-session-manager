// Resolves the deployed StrandsS3A2aAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['StrandsS3A2aAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'StrandsS3A2aAgent' found in runtime configuration (application ${application}).`,
    );
  }
