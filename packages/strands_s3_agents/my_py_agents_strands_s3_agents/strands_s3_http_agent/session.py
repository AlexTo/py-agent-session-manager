...
def get_session_manager() -> SessionManager | None:
    ...
    if os.environ.get("LOCAL_DEV") == "true":
        return FileSessionManager(session_id=session_id, storage_dir="../../tmp/agents/strands/strands-s3-http-agent")
    config = get_agentcore_runtime_config()
    bucket_name = (config.get("agentRuntimes") or {}).get("StrandsS3HttpAgent", {}).get("session", {}).get("bucketName")
    if not bucket_name:
