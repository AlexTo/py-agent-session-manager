...
class _A2AClient:
    ...
    async def _invoke(self, prompt: str) -> str:
        ...
        async with httpx.AsyncClient(auth=shared_client.auth, timeout=shared_client.timeout) as httpx_client:
            card = await A2ACardResolver(httpx_client=httpx_client, base_url=self._url).get_agent_card()
...
class AgentCoreA2aClientLangChain:
    ...
    @staticmethod
    def with_jwt_auth(agent_runtime_arn: str, access_token_provider: Callable[[], str]) -> _A2AClient:
        """Bearer-authenticated client for a Bedrock AgentCore runtime."""
        return _A2AClient(*AgentCoreA2aClientConfig.with_jwt_auth(agent_runtime_arn, access_token_provider))
