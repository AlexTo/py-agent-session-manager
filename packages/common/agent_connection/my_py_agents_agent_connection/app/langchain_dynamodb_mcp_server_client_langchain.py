...
class LangchainDynamodbMcpServerClientLangChain:
    """Loads LangChain tools from the LangchainDynamodbMcpServer MCP server."""

    @staticmethod
    def create() -> list[BaseTool]:
        if os.environ.get("LOCAL_DEV") == "true":
            return AgentCoreMCPClientLangChain.without_auth("http://localhost:8004/mcp")
        config = get_agentcore_runtime_config()
        agent_runtime = config.get("agentRuntimes", {}).get("LangchainDynamodbMcpServer")
        agent_runtime_arn = agent_runtime.get("arn") if agent_runtime else None
        if not agent_runtime_arn:
            raise RuntimeError(
                "No connected MCP server runtime named 'LangchainDynamodbMcpServer' found in runtime configuration."
            )
