...
class StrandsS3A2aAgentClientStrands:
    ...
    def create() -> A2AAgent:
        ...
        agent_runtime = config.get("agentRuntimes", {}).get("StrandsS3A2aAgent")
        agent_runtime_arn = agent_runtime.get("arn") if agent_runtime else None
        if not agent_runtime_arn:
            raise RuntimeError("No connected agent runtime named 'StrandsS3A2aAgent' found in runtime configuration.")
