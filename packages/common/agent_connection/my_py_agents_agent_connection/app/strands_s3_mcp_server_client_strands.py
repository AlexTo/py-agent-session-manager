...
class StrandsS3McpServerClientStrands:
    """Strands client for the StrandsS3McpServer MCP server."""
    ...
    def create() -> MCPClient:
        ...
        agent_runtime = config.get("agentRuntimes", {}).get("StrandsS3McpServer")
        agent_runtime_arn = agent_runtime.get("arn") if agent_runtime else None
        if not agent_runtime_arn:
            raise RuntimeError(
                "No connected MCP server runtime named 'StrandsS3McpServer' found in runtime configuration."
            )
