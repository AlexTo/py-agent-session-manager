...
export type LangchainS3AguiAgentProps = Omit<
  RuntimeProps,
  | 'runtimeName'
...
export class LangchainS3AguiAgent
  extends Construct
  implements IGrantable, IConnectable
{
  ...
  public readonly agentName = 'langchain-s3-agui-agent';
  /** Inbound auth — a fronting Gateway uses this to pick its outbound credential. */
  public readonly auth = 'iam';

  constructor(scope: Construct, id: string, props?: LangchainS3AguiAgentProps) {
    super(scope, id);
    ...
    const bundleDir = path.join(
      ...
      'dist/packages/langchain_s3_agents/docker/langchain-s3-agui-agent',
    );
    ...
    this.agentCoreRuntime = new Runtime(this, 'LangchainS3AguiAgent', {
    ...
    rc.set('connection', 'agentRuntimes', {
      ...
      LangchainS3AguiAgent: this.agentCoreRuntime.agentRuntimeArn,
    });
  }
