...
export type StrandsS3A2aAgentProps = Omit<
  RuntimeProps,
  | 'runtimeName'
...
export class StrandsS3A2aAgent
  extends Construct
  implements IGrantable, IConnectable
{
  ...
  public readonly agentName = 'strands-s3-a2a-agent';
  /** Inbound auth — a fronting Gateway uses this to pick its outbound credential. */
  public readonly auth = 'iam';

  constructor(scope: Construct, id: string, props?: StrandsS3A2aAgentProps) {
    super(scope, id);
    ...
    const bundleDir = path.join(
      ...
      'dist/packages/strands_s3_agents/docker/strands-s3-a2a-agent',
    );
    ...
    this.agentCoreRuntime = new Runtime(this, 'StrandsS3A2aAgent', {
