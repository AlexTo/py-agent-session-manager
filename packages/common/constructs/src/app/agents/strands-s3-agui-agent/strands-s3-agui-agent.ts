...
export type StrandsS3AguiAgentProps = Omit<
  RuntimeProps,
  | 'runtimeName'
...
export class StrandsS3AguiAgent
  extends Construct
  implements IGrantable, IConnectable
{
  ...
  public readonly agentName = 'strands-s3-agui-agent';
  /** Inbound auth — a fronting Gateway uses this to pick its outbound credential. */
  public readonly auth = 'iam';

  constructor(scope: Construct, id: string, props?: StrandsS3AguiAgentProps) {
    super(scope, id);
    ...
    const bundleDir = path.join(
      ...
      'dist/packages/strands_s3_agents/docker/strands-s3-agui-agent',
    );
    ...
    this.agentCoreRuntime = new Runtime(this, 'StrandsS3AguiAgent', {
    ...
    rc.set('connection', 'agentRuntimes', {
      ...
      StrandsS3AguiAgent: this.agentCoreRuntime.agentRuntimeArn,
    });
  }
