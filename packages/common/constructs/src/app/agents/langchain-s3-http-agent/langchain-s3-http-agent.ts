...
export type LangchainS3HttpAgentProps = Omit<
  RuntimeProps,
  | 'runtimeName'
...
export class LangchainS3HttpAgent
  extends Construct
  implements IGrantable, IConnectable
{
  ...
  public readonly agentName = 'langchain-s3-http-agent';
  /** Inbound auth — a fronting Gateway uses this to pick its outbound credential. */
  public readonly auth = 'iam';

  constructor(scope: Construct, id: string, props?: LangchainS3HttpAgentProps) {
    super(scope, id);
    ...
    const bundleDir = path.join(
      ...
      'dist/packages/langchain_s3_agents/docker/langchain-s3-http-agent',
    );
    ...
    this.agentCoreRuntime = new Runtime(this, 'LangchainS3HttpAgent', {
