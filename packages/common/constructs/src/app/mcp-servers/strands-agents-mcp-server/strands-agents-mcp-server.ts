...
export type StrandsAgentsMcpServerProps = Omit<
  RuntimeProps,
  | 'runtimeName'
...
export class StrandsAgentsMcpServer
  extends Construct
  implements IGrantable, IConnectable
{
  ...
  public readonly mcpServerName = 'strands-agents-mcp-server';

  constructor(
    scope: Construct,
    id: string,
    props?: StrandsAgentsMcpServerProps,
  ) {
    super(scope, id);
    ...
    const bundleDir = path.join(
      ...
      'dist/packages/strands_agents/docker/strands-agents-mcp-server',
    );
    ...
    this.agentCoreRuntime = new Runtime(this, 'StrandsAgentsMcpServer', {
