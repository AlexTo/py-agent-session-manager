...
def get_checkpointer() -> BaseCheckpointSaver:
    ...
    if os.environ.get("LOCAL_DEV") == "true":
        os.makedirs("../../tmp/agents/langchain/langchain-dynamodb-agui-agent", exist_ok=True)
        conn = aiosqlite.connect(
            os.path.join("../../tmp/agents/langchain/langchain-dynamodb-agui-agent", "checkpoints.sqlite")
        )
        return AsyncSqliteSaver(conn)
    config = get_agentcore_runtime_config()
    session_config = (config.get("agentRuntimes") or {}).get("LangchainDynamodbAguiAgent", {}).get("session", {})
