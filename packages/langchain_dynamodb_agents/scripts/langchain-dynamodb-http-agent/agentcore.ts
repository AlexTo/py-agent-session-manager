// Resolves the deployed LangchainDynamodbHttpAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['LangchainDynamodbHttpAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'LangchainDynamodbHttpAgent' found in runtime configuration (application ${application}).`,
    );
  }
