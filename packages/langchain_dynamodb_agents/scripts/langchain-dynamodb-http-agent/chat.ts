// Chat CLI for LangchainDynamodbHttpAgent (Python FastAPI / JSONL streaming), using the
...
import { LangchainDynamodbHttpAgent } from './generated/client.gen.js';
import {
...
class LangchainDynamodbHttpAgentAdapter implements ChatAdapter {
  private client!: LangchainDynamodbHttpAgent;

  async connect(url: string) {
    this.client = remote
      ? new LangchainDynamodbHttpAgent({
    ...
    return { agentName: 'LangchainDynamodbHttpAgent' };
  }
...
await chatLoop(new LangchainDynamodbHttpAgentAdapter(), process.env.URL ?? '');
