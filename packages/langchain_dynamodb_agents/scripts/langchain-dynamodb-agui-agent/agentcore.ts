// Resolves the deployed LangchainDynamodbAguiAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['LangchainDynamodbAguiAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'LangchainDynamodbAguiAgent' found in runtime configuration (application ${application}).`,
    );
  }
