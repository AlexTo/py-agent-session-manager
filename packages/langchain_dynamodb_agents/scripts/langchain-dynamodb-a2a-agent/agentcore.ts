// Resolves the deployed LangchainDynamodbA2aAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['LangchainDynamodbA2aAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'LangchainDynamodbA2aAgent' found in runtime configuration (application ${application}).`,
    );
  }
