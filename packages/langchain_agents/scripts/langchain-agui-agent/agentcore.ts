// Resolves the deployed LangchainAguiAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['LangchainAguiAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'LangchainAguiAgent' found in runtime configuration (application ${application}).`,
    );
  }
