// Resolves the deployed LangchainA2aAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['LangchainA2aAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'LangchainA2aAgent' found in runtime configuration (application ${application}).`,
    );
  }
