// Resolves the deployed LangchainHttpAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['LangchainHttpAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'LangchainHttpAgent' found in runtime configuration (application ${application}).`,
    );
  }
