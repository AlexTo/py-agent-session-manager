// Resolves the deployed StrandsHttpAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['StrandsHttpAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'StrandsHttpAgent' found in runtime configuration (application ${application}).`,
    );
  }
