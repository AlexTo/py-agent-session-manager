// Resolves the deployed StrandsAguiAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['StrandsAguiAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'StrandsAguiAgent' found in runtime configuration (application ${application}).`,
    );
  }
