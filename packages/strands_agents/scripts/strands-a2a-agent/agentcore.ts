// Resolves the deployed StrandsA2aAgent agent from runtime config and authenticates requests to it.
...
> => {
  ...
  const arn = config?.agentRuntimes?.['StrandsA2aAgent']?.arn;
  if (!arn) {
    throw new Error(
      `No deployed agent named 'StrandsA2aAgent' found in runtime configuration (application ${application}).`,
    );
  }
